...
def _drop_explicit_zeros(dist_matrix):
    """
    CSR view of a predicted-distance matrix with explicit zeros removed.

    `Fit_cord_multi` saves the prediction with `pair_dist[pair_dist == 0] = np.inf`
    applied first, so a zero means "not a neighbor", never "distance 0". Dropping
    stored zeros makes the sparse matrix mean exactly what the dense matrix meant:
    a stored entry is a finite distance, everything else is infinite.
    """
    M = dist_matrix.tocsr()
...
def _knn_indices_sparse(dist_matrix, knn):
    """
    Row-wise indices of the `knn` smallest stored distances, straight from CSR.

    Equivalent to `np.argsort(dense, axis=1)[:, :knn]` when the non-stored entries
    are infinite, but without ever materializing the dense matrix.

    Returns
    -------
    knn_indices : np.ndarray of int32, shape (n, knn)
        Neighbor indices, right-padded with -1 for rows holding fewer than `knn`
        predicted neighbors.
    n_used : np.ndarray of int64, shape (n,)
        Number of neighbors actually found per row.
    """
    M = _drop_explicit_zeros(dist_matrix)
    n = M.shape[0]
    indptr, indices, data = M.indptr, M.indices, M.data

    knn_indices = np.full((n, knn), -1, dtype=np.int32)
    n_used = np.zeros(n, dtype=np.int64)
    for i in range(n):
        s, e = indptr[i], indptr[i + 1]
        idx = indices[s:e]
        if idx.size and (idx == i).any():   # never count a cell as its own neighbor
            keep = idx != i
            idx = idx[keep]
            val = data[s:e][keep]
        else:
        ...
        if idx.size == 0:
            continue
        if idx.size > knn:
            idx = idx[np.argpartition(val, knn - 1)[:knn]]
        knn_indices[i, :idx.size] = idx
        n_used[i] = idx.size
    return knn_indices, n_used


def _cluster_fraction_matrix(clusters, cluster_uniq, knn_indices, n_used):
    """
    Fraction of each cluster among every cell's neighbors, vectorized.

    Same quantity as `np.sum(neighbor_clusters == uc) / knn`, computed with one
    `np.bincount` instead of an n x n_clusters Python loop. Cells whose label
    matches no entry of `cluster_uniq` contribute to no count, exactly as the
    elementwise comparison did.
    """
    n, k = knn_indices.shape
    n_clust = len(cluster_uniq)

    codes = np.full(len(clusters), -1, dtype=np.int64)
    for j, uc in enumerate(cluster_uniq):
        codes[clusters == uc] = j

    valid = knn_indices >= 0
    nb_codes = np.where(valid, codes[np.where(valid, knn_indices, 0)], -1)
    ok = valid & (nb_codes >= 0)
    rows = np.broadcast_to(np.arange(n, dtype=np.int64)[:, None], (n, k))
    flat = rows[ok] * n_clust + nb_codes[ok]

    counts = np.bincount(flat, minlength=n * n_clust).reshape(n, n_clust).astype(float)
    return counts / np.maximum(n_used, 1)[:, None]
...
def cn_cluster_dist(loc_pred, dist_matrix, ct_key = "Cluster", n_clust=10, knn=100, random_state=1):
    ...
    dist_matrix : np.ndarray or scipy.sparse matrix
        ...
        A sparse matrix is read directly, as saved by `Fit_cord_multi`: a stored
        entry is a predicted neighbor distance and everything else counts as
        infinite. It needs neither densification nor the
        `dist[dist == 0] = np.inf` step. A row holding fewer than `knn` predicted
        neighbors uses all of them (the dense path pads such rows with arbitrary
        infinitely-distant cells instead).
    ct_key : str, optional
    ...
    if issparse(dist_matrix):
        # A stored entry is a predicted neighbor, anything else is infinitely far,
        # so the kNN of a row are the smallest stored values in it. No dense
        # n x n matrix is built and no inf substitution is needed.
        knn_indices, n_used = _knn_indices_sparse(dist_matrix, knn)
    else:
        ...
        n_used = np.full(n, knn_indices.shape[1], dtype=np.int64)
    ...
    cluster_fraction_matrix = _cluster_fraction_matrix(clusters, cluster_uniq, knn_indices, n_used)
...
def cn_cluster(loc_pred, loc_matrix, ct_key = "Cluster", n_clust=10, knn=100, random_state=1):
    ...
    **coordinate matrix** and calls the appropriate clustering function.
    ...
    loc_matrix : np.ndarray or scipy.sparse matrix
        Either a square distance matrix (n_samples x n_samples) or a coordinate matrix
        (n_samples x 2 or n_samples x 3) representing spatial locations. A sparse
        distance matrix, as saved by `Fit_cord_multi`, is passed straight through.
    ct_key : str, optional
        Column in `loc_pred` containing initial cluster labels. Default is "Cluster".
    n_clust : int, optional
