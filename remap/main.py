...
# def _preprocess_query_only(Rdata, Qdata):
#     """
#     Query half of `preprocess`, for the prediction-only path.

#     `preprocess` concatenates and normalizes the whole reference on top of the
#     query, but only two of its effects reach `Pred_relative`: the ordered list of
#     shared features, and the normalization of the query itself -- which uses the
#     query's own gene mean/sd (see `get_zscore(sc_data)` in `util.preprocess`),
#     not the reference's. The feature order depends solely on the variable
#     *names*, so it is reproduced here without reading a single reference
#     expression value.

#     Returns the preprocessed query; the reference is left untouched.
#     """
#     if isinstance(Rdata, list):
#         common = set(Rdata[0].var_names)
#         for ref in Rdata[1:]:
#             common &= set(ref.var_names)
#         ref_names = np.array(list(common), dtype=str)
#     else:
#         ref_names = np.asarray(Rdata.var_names, dtype=str)

#     # preprocess() ends in np.intersect1d(st.var.index, sc.var.index), whose
#     # output is sorted and unique -- the same order the model was trained on.
#     features = np.intersect1d(ref_names, np.asarray(Qdata.var_names, dtype=str))
#     Qdata = Qdata[:, features].copy()
#     lib_norm(Qdata)
#     get_zscore(Qdata)
#     return Qdata


def Fit_cord_multi(Rdata, location_data, Qdata, path_name, source_key = "source", equal_size = False, grid_size = 200, batch_train = 1024, batch_test = 8192,
                   sample_pairs = 100000, full_pairwise = False, neighbor_fraction=0.1, harmony = True, 
                 hidden_dims = [400, 100, 100], save = True, predict = True, num_workers = 8):
    """
    ...
    Rdata : AnnData or list of AnnData
        ...
        May be None on the prediction-only path (see Notes).
    location_data : np.ndarray or list of np.ndarray
        Spatial coordinates corresponding to Rdata cells.
        Training only; ignored when the model already exists.
    Qdata : AnnData
    ...
    dist_pred : np.ndarray
        ...
        or None if `predict` is False.

    Notes
    -----
    Training happens only when `{path_name}/remap_rel.pt` is absent. When it is
    already there the function takes a prediction-only path that skips every step
    whose sole purpose is to build the training set:

    * the capture weights from `st_frac` -- they are the per-pair loss weights of
      `Fit_relative` and are never read by `Pred_relative`. This is by far the
      most expensive of the skipped steps: it concatenates all captures with the
      query, runs PCA + neighbors, then a BallTree kNN query.
    * `norm_cord` / `norm_cord_scale` / `select_sample` -- they only rescale and
      grid-subsample the *reference* coordinates used to form training pairs;
      `Pred_relative` feeds a zero coordinate matrix.
    * the reference half of `preprocess`, and the reference `covet` / spatial
      bookkeeping. Only the query is normalized, by `_preprocess_query_only`,
      which reproduces the trained feature order from variable names alone.

    On that path the reference is needed only for its `var_names`, and not at all
    when `harmony=True` and `{path_name}/sc_harmony.npy` is cached (Harmony
    correction sets `norm = False`, so no feature selection happens at training
    time either) -- so `Rdata=None, location_data=None` is then accepted.
    """
    ...
    # Training happens only if no model has been saved yet. Everything that
    # exists purely to build the training set is skipped otherwise.
    train = not os.path.exists(f"{path_name}/remap_rel.pt")
    if not train:
        print("Found a trained model, skipping training.")
        if not predict:
            return None
    elif Rdata is None:
        raise ValueError("Rdata is required to train a model.")
    ...
    norm = True
    if harmony:
        st_file = f"{path_name}/st_harmony.npy"; sc_file = f"{path_name}/sc_harmony.npy"
        if os.path.exists(sc_file) and (os.path.exists(st_file) or not train):
            print("Loading Harmony corrected expression")
            if train:   # the reference side is only needed to build training pairs
                Rdata.obsm["corrected"] = np.load(st_file)
            Qdata.obsm["corrected"] = np.load(sc_file)
        elif Rdata is not None:
            Rdata, Qdata = run_harmony(Rdata, Qdata, norm = True, st_all = False)
            np.save(st_file, Rdata.obsm["corrected"])
            np.save(sc_file, Qdata.obsm["corrected"])
        else:
            raise ValueError(f"harmony=True but {sc_file} is missing and no Rdata "
                             f"was given to recompute the correction.")
        norm = False

    # `norm` is False after Harmony, and preprocess() is gated on it below, so the
    # reference is only needed for training or to derive the shared feature list.
    if train or norm:
        if Rdata is None:
            raise ValueError("Rdata is required to determine the feature order "
    ...
    if train:
        ...
        sample_pairs = sample_pairs * len(Rdata)  # Adjust sample pairs based on the number of reference datasets
        ...
        # The normalization preprocess() would apply to the query, without
        # touching the reference expression matrix.
        # Qdata = _preprocess_query_only(Rdata, Qdata)
        Rdata, Qdata = preprocess(Rdata, Qdata, normalize = True, scale = True)
...
def Predict_cord_rel(Rdata, Qdata, path_name, batch_size = 8192, full_pairwise = False, neighbor_fraction=0.1, save = True, filename = "remap_rel_dist"):
    """
    ...
    """
    norm = True
    if os.path.exists(f"{path_name}/st_harmony.npy") and os.path.exists(f"{path_name}/sc_harmony.npy"):
        print("Loading Harmony corrected expression")
        Rdata.obsm["corrected"] = np.load(f"{path_name}/st_harmony.npy")
        Qdata.obsm["corrected"] = np.load(f"{path_name}/sc_harmony.npy")
        norm = False
